...
private _fnc_getVehMarkername = {
    format ["IMF_Warmup_%1_%2", floor ((_this select 0) * 10), floor ((_this select 1) * 10)];
};
    ...
    if ((_x select 0) == "V") then {
        ...
        if (_side in _friendlysides) then {
            ...
            if (count _inv > 0) then {
                _inventorytext = _inventorytext + format ["<br/><img image='%2' height=24/> <font color='#c7861b'>%1</font><br/>",
                format ["%1", gettext (configFile >> "Cfgvehicles" >> _class >> "displayname") ], _pic];
